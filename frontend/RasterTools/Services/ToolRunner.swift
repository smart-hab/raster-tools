...
class ToolRunner {
    ...
    /// Extract a zip archive with `ditto`, which handles the large archives both collectors
    /// download without pulling in an unzip dependency.
    func runDitto(source: String, destination: String) async throws {
        let process = Process()
