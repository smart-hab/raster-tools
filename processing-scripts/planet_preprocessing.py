...
input_path = "D:/Downloads/BellisleBay/"
save_dir = "D:/StudyAreas/BelleisleBay/Cropped/"
#ce_path = "C:/Users/cathe/Documents/StudyAreas/Shapefiles/DarlingsLake.shp"
ce_path = "D:/StudyAreas/Shapefiles/BelleIsleBay.geojson"
start_date = 'string'
end_date = 'string'
...
def get_date(filename):
    spl_word = '20'
    endstring = spl_word + (filename.split(spl_word,1))[1]
    day = endstring[:8]
...
for file in files:
    ...
    dayfiles = glob.glob(filepath + '/**/*.tif', recursive=True)
    
    #dayfiles = [x for x in dayfiles if 'udm' not in x]
    
    im_path = dayfiles[0]
    udm_path = dayfiles[1]
    meta_path = glob.glob(input_path + file +
                          '**/*composite_metadata.json', recursive=True)
    ...
    print("Clipping...")
    rast_clipped = rast_area.rio.clip(crop_extent.geometry.apply(mapping))
    ...
    outTIF = save_dir + date + "_" + bands + ".tif"
