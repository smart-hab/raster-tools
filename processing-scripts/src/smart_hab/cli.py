...
def plot() -> None:
  ...
  args = p.parse_args()
  input = Path(args.cwd) / cast(str, args.input)
  output = Path(args.cwd) / cast(str, args.output) if args.output else input.parent / f'{input.stem}.png'
...
def norm_diff() -> None:
  ...
  output = Path(args.cwd) / cast(str, args.output) if args.output else input.parent / f'{input.stem}_{new_name}.tif'
  match cast(list[int], args.bands):
    case [a,b]:
      bands = (a,b)
    case _:
