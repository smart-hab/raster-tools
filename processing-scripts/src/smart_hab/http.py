...
async def plot(
  ...
  match bands:
    ...
    case _:
      raise HTTPException(status_code=400, detail=f'Invalid band selection: ({bands}). Must be 1 or 3 bands.')
  return JSONResponse({ 'output': str(output) })
...
def server() -> None:
  ...
  p.add_argument('--host', type=str, help='Host to bind to (default: 127.0.0.1)', default='127.0.0.1')
