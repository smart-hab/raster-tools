...
def raster_norm_diff(
    raster: xarray.DataArray, band1: int, band2: int, epsilon: float = 1e-10
) -> xarray.DataArray:
...
def raster_robust_norm(
    raster: xarray.DataArray, low: float = 0, high: float = 100
) -> xarray.DataArray:
    valid = numpy.isfinite(raster.values)
    vmin, vmax = typing.cast(
        tuple[float, float], numpy.percentile(raster.values[valid], [low, high])
    )
